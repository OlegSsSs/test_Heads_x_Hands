...
public class Main {
    public static void main(String[] args) {
        BaseHero.Dice dice = new BaseHero.Dice();
        Player player = new Player("Игрок", 15, 20, 100, 1, 6, 4, 0.3);
        Monsters monsters = new Monsters("Монстр", 16, 18, 120, 1, 6);
        ...
        while (player.isAlive() && monsters.isAlive()) {
            player.attack(monsters, dice);
            if (monsters.isAlive()) {
                monsters.attack(player, dice);
            }
            player.heal();
        }
        if (player.isAlive()) {
